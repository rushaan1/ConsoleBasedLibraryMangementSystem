    ...
    internal class LibraryManagement
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    switch (option)
                    {
                        ...
                        case 12:
                            ...
                            break;
                        case 13:
                            var account = ctx.Administrators.Where(a => a.Username == username).First();
                            ctx.Remove(account);
                            ctx.SaveChanges();
                            Console.WriteLine("\nSuccessfully deleted current administrator account!\n");
                            exit = true;
                            break;
                ...
                catch (FormatException ex) { Console.WriteLine($"An Exception occured with message: {ex.Message}\nThis usually happens when empty input, letter or words in place of number or input with only white spaces is provided.\n"); }
                catch (InvalidOperationException ex2) { Console.WriteLine($"\nAn Exception occured with message: {ex2.Message}\nThis usually happens when invalid Id, username or phone number is provided which does not exists.\n"); }
                catch (Exception ex3) { Console.WriteLine($"\nAn Exception occured with message: {ex3.Message}\nThis is an unknown exception.\n"); };
